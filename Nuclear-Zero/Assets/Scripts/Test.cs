...
public class Test : MonoBehaviour
{
    ...
    //        GPGSBinder.Inst.UnlockAchievement(GPGSIds.achievement_gamelogin, success => log = $"{success}");

    //    if (GUILayout.Button("UnlockAchievement_two"))
    //        GPGSBinder.Inst.UnlockAchievement(GPGSIds.achievement_gamethree, success => log = $"{success}");

    //    if (GUILayout.Button("IncrementAchievement_three"))
    //        GPGSBinder.Inst.IncrementAchievement(GPGSIds.achievement_gametwo, 1, success => log = $"{success}");
    ...
    //        GPGSBinder.Inst.IncrementEvent(GPGSIds.event_evnet, 1);

    //    if (GUILayout.Button("LoadEvent_event"))
    //        GPGSBinder.Inst.LoadEvent(GPGSIds.event_evnet, (success, iEvent) =>
    //        {
